...
namespace kvstore {
template <typename Comparator> class SkipList {
public:
  struct Node {
    ...
    const int height;
    std::atomic<Node *>
        next_[1]; // Flexible array member, size determined at allocation
    ...
    static Node *NewNode(const Slice &k, const Slice &v, int h) {
      void *mem = operator new(sizeof(Node) +
                               (h - 1) * sizeof(std::atomic<Node *>));
      return new (mem) Node(k, v, h);
    }

  private:
    // The constructor is private; nodes can only be created via NewNode
    // factory.
...
SkipList<Comparator>::SkipList(const Comparator &cmp)
    : comparator_(cmp),
      ...
      max_height_(1), rand_generator_(std::random_device{}()),
...
template <typename Comparator> SkipList<Comparator>::~SkipList() {
...
template <typename Comparator> int SkipList<Comparator>::RandomHeight() {
...
SkipList<Comparator>::FindGreaterOrEqual(const Slice &key, Node **prev) const {
  Node *x = head_;
